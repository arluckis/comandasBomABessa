...
export default function Login({ getHoje, setSessao, setScenePhase, temaNoturno }) {
  ...
  const handleTyping = (e, field) => {
    ...
    const newParticle = { id: Date.now() + Math.random(), x: Math.random() * 100, y: Math.random() * 100, size: Math.random() * 1.5 + 0.5, opacity: Math.random() * 0.5 + 0.2 };
    setTypingParticles(prev => [...prev.slice(-10), newParticle]);
  };

  useEffect(() => {
    if (typingParticles.length > 0) { const timer = setTimeout(() => setTypingParticles([]), 1000); return () => clearTimeout(timer); }
  }, [typingParticles]);
  ...
  const cardStyle = isLight ? 'bg-white/90 backdrop-blur-2xl border-black/[0.05] shadow-[0_40px_80px_rgba(0,0,0,0.08)]' : 'bg-white/[0.02] backdrop-blur-md border-white/[0.08] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)]';
  const inputStyle = isLight ? 'bg-black/[0.03] text-zinc-900 placeholder:text-zinc-400 focus:bg-white focus:border-black/20 focus:ring-4 ring-black/5 border-transparent' : 'bg-black/30 text-white placeholder:text-zinc-600 focus:border-white/30 focus:ring-1 focus:ring-white/10 border-white/10';
  const labelStyle = isLight ? 'text-zinc-400 font-bold' : 'text-zinc-500 font-medium';
    ...
    <div className="min-h-[100dvh] flex flex-col lg:flex-row w-full font-sans selection:bg-black/10 selection:text-black relative z-10 bg-transparent">
      ...
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden mix-blend-screen">
        {typingParticles.map(particle => (
          <div key={particle.id} className={`absolute rounded-full animate-in fade-in zoom-in duration-700 ease-out ${isLight ? 'bg-blue-500/20' : 'bg-blue-200/50'}`} style={{ left: `${particle.x}%`, top: `${particle.y}%`, width: `${particle.size}px`, height: `${particle.size}px`, opacity: particle.opacity, animation: 'fadeOutParticle 1s forwards' }} />
        ))}
      </div>
      ...
      <div className="w-full lg:w-[55%] flex flex-col justify-end lg:justify-center p-8 lg:p-24 relative z-20 h-[35dvh] lg:h-auto">
        <div className="animate-in fade-in slide-in-from-left-4 duration-1000">
          <div className="flex items-center gap-3 mb-6 pointer-events-none">
            <div className={`h-[1px] w-8 ${isLight ? 'bg-zinc-300' : 'bg-white/20'}`}></div>
            <span className={`font-mono tracking-[0.2em] text-[10px] uppercase font-bold ${isLight ? 'text-zinc-400' : 'text-zinc-400'}`}>AROX Core v3.0.4</span>
          </div>
          
          <h1 className={`text-[2rem] lg:text-[4.5rem] font-bold leading-[1.05] tracking-tight drop-shadow-sm pointer-events-none ${isLight ? 'text-zinc-900' : 'text-white drop-shadow-lg'}`}>
            Domínio <br className="hidden lg:block"/> em tempo real.
          </h1>
          <p className={`mt-4 lg:mt-6 text-[14px] lg:text-[16px] leading-relaxed max-w-md pointer-events-none ${isLight ? 'text-zinc-500 font-medium' : 'text-zinc-400 font-light'}`}>
          ...
          <div className="mt-8 flex gap-4 pointer-events-auto">
            <button onClick={() => window.open('https://wa.me/5584994229126', '_blank')} className={`px-6 py-3 border rounded-xl text-[13px] font-bold flex items-center gap-2 group backdrop-blur-sm transition-all duration-300 ${isLight ? 'bg-white/50 border-black/10 text-zinc-800 hover:bg-white shadow-sm' : 'bg-white/5 border-white/10 text-white hover:bg-white/10 hover:border-white/20 hover:shadow-[0_0_20px_rgba(255,255,255,0.05)]'}`}>
          ...
          <div className={`hidden lg:flex gap-10 text-[11px] font-mono pt-8 mt-12 border-t pointer-events-none ${isLight ? 'text-zinc-400 border-black/5' : 'text-zinc-500 border-white/5'}`}>
            <div>
               <span className={`block font-bold mb-1 ${isLight ? 'text-zinc-300' : 'text-white/30'}`}>STATUS</span>
               <span className="text-emerald-500 flex items-center gap-2 font-bold"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span> Operacional</span>
            </div>
            <div>
               <span className={`block font-bold mb-1 ${isLight ? 'text-zinc-300' : 'text-white/30'}`}>ENCRYPTION</span>
               <span className={`font-bold ${isLight ? 'text-zinc-600' : 'text-zinc-300'}`}>AES-256 E2E</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="w-full lg:w-[45%] flex flex-col justify-start lg:justify-center items-center lg:items-start p-6 lg:p-20 relative z-20 h-[65dvh] lg:h-auto">
        ...
        <div className={`w-full max-w-[400px] border p-8 lg:p-10 rounded-[32px] transition-all duration-700 animate-in fade-in slide-in-from-bottom-8 delay-100 ${cardStyle}`}>
          
          <div className="mb-8">
            <h2 className={`text-[1.25rem] font-bold tracking-tight ${isLight ? 'text-zinc-900' : 'text-zinc-100 font-medium'}`}>
            ...
            <p className={`mt-1.5 text-[13px] ${isLight ? 'text-zinc-500 font-medium' : 'text-zinc-500 font-light'}`}>
               {stepTrocaSenha ? 'Crie uma nova senha para acessar sua conta.' : (!mostrarFormPadrao && lastLogin) ? 'Autenticar sessão protegida.' : 'Assinatura digital exigida.'}
            </p>
          </div>
          ...
          {stepTrocaSenha ? (
            <form onSubmit={salvarNovaSenha} className="space-y-5">
              <div className="space-y-3">
                <input type="password" placeholder="Nova Senha Master" className={`w-full px-4 py-3.5 outline-none text-[14px] font-bold rounded-xl transition-all duration-300 border ${inputStyle}`} value={novaSenha} onChange={e => handleTyping(e, 'novaSenha')} onFocus={() => { setActiveInput('novaSenha'); if(setScenePhase) setScenePhase('sync'); }} onBlur={() => { setActiveInput(null); if(setScenePhase) setScenePhase('reveal'); }} autoFocus />
                <input type="password" placeholder="Confirmar Senha Master" className={`w-full px-4 py-3.5 outline-none text-[14px] font-bold rounded-xl transition-all duration-300 border ${inputStyle} ${confirmTouched && !matchPasswords && !isLight ? '!border-red-500/50' : ''} ${confirmTouched && !matchPasswords && isLight ? '!border-red-500 !ring-red-100' : ''}`} value={confirmarNovaSenha} onChange={e => handleTyping(e, 'confirmarNovaSenha')} onFocus={() => setActiveInput('confirm')} onBlur={() => setActiveInput(null)} />
              </div>

              <div className={`pt-3 rounded-xl p-4 border ${isLight ? 'bg-zinc-50 border-black/5' : 'bg-white/[0.02] border-white/5'}`}>
                ...
                <div className="grid grid-cols-2 gap-y-2 gap-x-2">
                  <div className={`flex items-center gap-2 text-[11px] font-bold ${isLight ? 'text-zinc-500' : 'font-mono text-zinc-400'}`}><CheckIcon active={hasLength} /> Mínimo 8 Carac.</div>
                  <div className={`flex items-center gap-2 text-[11px] font-bold ${isLight ? 'text-zinc-500' : 'font-mono text-zinc-400'}`}><CheckIcon active={hasNumber} /> Número(s)</div>
                  <div className={`flex items-center gap-2 text-[11px] font-bold ${isLight ? 'text-zinc-500' : 'font-mono text-zinc-400'}`}><CheckIcon active={hasSpecial} /> Símbolo(s)</div>
                  <div className={`flex items-center gap-2 text-[11px] font-bold ${isLight ? 'text-zinc-500' : 'font-mono text-zinc-400'}`}><CheckIcon active={matchPasswords} /> Senhas Iguais</div>
                </div>
              </div>

              <div className="flex items-start gap-3 px-1 py-1 mt-2">
                <div className="relative flex items-center mt-0.5">
                  <input type="checkbox" id="termos" checked={aceitouTermos} onChange={e => setAceitouTermos(e.target.checked)} className={`peer appearance-none w-4 h-4 border rounded-[4px] transition-all cursor-pointer ${isLight ? 'border-black/20 bg-black/5 checked:bg-zinc-900 checked:border-zinc-900' : 'border-white/20 bg-black/30 checked:bg-white checked:border-white'}`} />
                  <svg className={`absolute w-3 h-3 pointer-events-none opacity-0 peer-checked:opacity-100 top-0.5 left-0.5 transition-opacity ${isLight ? 'text-white' : 'text-black'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={4} d="M5 13l4 4L19 7" /></svg>
                </div>
                <label htmlFor="termos" className={`text-[11.5px] font-medium leading-[1.4] select-none cursor-pointer ${isLight ? 'text-zinc-600' : 'text-zinc-400 font-light'}`}>
                  Confirmo que li e aceito as <button type="button" onClick={() => setModalContent({ open: true, title: 'Termos de Operação', text: policies.termos })} className={`transition-colors underline underline-offset-2 font-bold ${isLight ? 'text-zinc-900' : 'text-zinc-200 hover:text-white'}`}>Políticas Operacionais</button> e a <button type="button" onClick={() => setModalContent({ open: true, title: 'Privacidade de Dados', text: policies.privacidade })} className={`transition-colors underline underline-offset-2 font-bold ${isLight ? 'text-zinc-900' : 'text-zinc-200 hover:text-white'}`}>Privacidade de Dados</button>.
                </label>
              </div>

              {erro && <p className="text-[12px] text-red-500 font-bold animate-in fade-in pt-1">{erro}</p>}
              
              <button type="submit" disabled={loadingLogin || pwdScore < 4 || !aceitouTermos} className={`w-full py-4 text-[12px] font-black uppercase tracking-widest transition-all duration-300 mt-4 rounded-xl ${pwdScore === 4 && aceitouTermos ? (isLight ? 'bg-zinc-900 text-white hover:bg-black shadow-[0_8px_20px_rgba(0,0,0,0.15)] active:scale-[0.98]' : 'bg-zinc-100 text-black hover:bg-white hover:shadow-[0_0_20px_rgba(255,255,255,0.2)] active:scale-[0.98]') : (isLight ? 'bg-black/5 text-zinc-400 cursor-not-allowed' : 'bg-white/5 text-zinc-500 cursor-not-allowed border border-white/5')}`}>
                {loadingLogin ? <LoadingDots isLight={!isLight} /> : 'Ativar Credencial'}
              </button>
            </form>

          ) : (!mostrarFormPadrao && lastLogin) ? (
            <div className="w-full flex flex-col">
              <div className={`flex items-center gap-4 mb-8 p-4 rounded-[16px] border ${isLight ? 'bg-white shadow-sm border-black/5' : 'bg-black/30 border-white/5'}`}>
                <div className={`w-12 h-12 rounded-xl overflow-hidden flex-shrink-0 flex items-center justify-center border ${isLight ? 'bg-zinc-50 border-black/5' : 'bg-zinc-900 border-white/5'}`}>
                ...
                <div className="overflow-hidden">
                  <p className={`text-[14px] font-bold truncate ${isLight ? 'text-zinc-900' : 'text-zinc-100 font-medium'}`}>{lastLogin.nome_usuario || 'Administrador'}</p>
                  <p className={`text-[12px] truncate mt-0.5 ${isLight ? 'text-zinc-500 font-bold' : 'text-zinc-500 font-mono'}`}>{lastLogin.nome_empresa}</p>
                </div>
              </div>
              {erro && <p className="text-[12px] text-red-500 font-bold animate-in fade-in pb-4">{erro}</p>}
              <div className="space-y-3">
                <button onClick={loginComContaSalva} disabled={loadingLogin} className={`w-full py-4 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all duration-300 active:scale-[0.98] disabled:opacity-50 ${isLight ? 'bg-zinc-900 text-white shadow-lg shadow-black/10' : 'bg-zinc-100 text-black hover:bg-white'}`}>
                  {loadingLogin ? <LoadingDots isLight={!isLight} /> : 'Acessar Instância'}
                </button>
                <button onClick={() => setMostrarFormPadrao(true)} className={`w-full py-3.5 rounded-xl text-[12px] font-bold transition-all duration-300 ${isLight ? 'text-zinc-500 hover:text-zinc-900 hover:bg-black/5' : 'text-zinc-400 hover:text-zinc-200 hover:bg-white/5 border border-transparent hover:border-white/10'}`}>
            ...
            <form className="space-y-5" onSubmit={fazerLogin}>
              <div className="space-y-3">
                <div>
                  <label className={`block text-[10px] uppercase tracking-widest mb-2 ml-1 ${labelStyle}`}>Assinatura Digital</label>
                  <input id="email" type="email" placeholder="nome@empresa.com" className={`w-full px-5 py-4 rounded-2xl outline-none border transition-all duration-300 font-bold text-[14px] ${inputStyle}`} value={credenciais.email} onChange={e => handleTyping(e, 'email')} onFocus={() => { setActiveInput('email'); if(setScenePhase) setScenePhase('sync'); }} onBlur={() => { setActiveInput(null); if(setScenePhase) setScenePhase('reveal'); }} autoFocus={!lastLogin} />
                </div>
                <div>
                  <label className={`block text-[10px] uppercase tracking-widest mb-2 ml-1 mt-4 ${labelStyle}`}>Chave de Segurança</label>
                  <input id="senha" type="password" placeholder="••••••••" className={`w-full px-5 py-4 rounded-2xl outline-none border transition-all duration-300 font-bold text-[14px] ${inputStyle}`} value={credenciais.senha} onChange={e => handleTyping(e, 'senha')} onFocus={() => setActiveInput('senha')} onBlur={() => setActiveInput(null)} />
                </div>
              </div>
              {erro && <p className="text-[12px] text-red-500 font-bold animate-in fade-in pt-1">{erro}</p>}
              <button type="submit" disabled={loadingLogin} className={`w-full py-4 mt-4 rounded-xl text-[12px] font-black uppercase tracking-[0.2em] transition-all duration-500 active:scale-[0.98] disabled:opacity-50 ${isLight ? 'bg-zinc-900 text-white hover:bg-black shadow-[0_10px_30px_rgba(0,0,0,0.15)]' : 'bg-zinc-100 text-black hover:bg-white shadow-[0_4px_14px_0_rgba(255,255,255,0.1)] hover:shadow-[0_6px_20px_rgba(255,255,255,0.15)]'}`}>
                {loadingLogin ? <LoadingDots isLight={!isLight} /> : 'Autorizar Conexão'}
              </button>
              {lastLogin && (
                 <div className="pt-4 text-center">
                   <button type="button" onClick={() => setMostrarFormPadrao(false)} className={`text-[12px] font-bold transition-colors duration-300 ${isLight ? 'text-zinc-400 hover:text-zinc-800' : 'text-zinc-500 hover:text-zinc-300 font-medium'}`}>
        ...
        <div className="w-full max-w-[400px] mt-8 text-center lg:text-left flex justify-between items-center px-2">
          <p className={`text-[10px] font-bold uppercase tracking-wider ${isLight ? 'text-zinc-400' : 'font-mono text-zinc-600'}`}>Arox Systems © {new Date().getFullYear()}</p>
          <div className={`flex gap-4 text-[10px] font-bold uppercase ${isLight ? 'text-zinc-400' : 'font-mono text-zinc-500'}`}>
             <button onClick={() => setModalContent({ open: true, title: 'Privacidade de Dados', text: policies.privacidade })} className={`transition-colors ${isLight ? 'hover:text-zinc-800' : 'hover:text-zinc-300'}`}>Privacidade</button>
             <button onClick={() => setModalContent({ open: true, title: 'Termos de Operação', text: policies.termos })} className={`transition-colors ${isLight ? 'hover:text-zinc-800' : 'hover:text-zinc-300'}`}>Termos</button>
          </div>
        </div>
